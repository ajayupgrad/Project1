...
public class UserController {
    ...
    public String registerUser(User user,Model mod) {

        //Call the password check method to validate pwd against a regular expression
        ...
        //If pwd doesnt satisfy the condition redirect to registration page and show the below error on screen

        String errMsg="Password must contain atleast 1 alphabet, 1 number & 1 special character";
        mod.addAttribute("User",user);
        mod.addAttribute("passwordTypeError",errMsg);
        return "users/registration";
...
// This method is called when user registers and it will check the pwd follows the specified pattern.

    private boolean passwordCheck(String pwd)
    {

        Pattern pattern;
        Matcher matcher;

        String PASSWORD_PATTERN ="((?=.*\\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$%]).{1,20})";
